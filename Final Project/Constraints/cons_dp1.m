function [ Phi, Phi_q, Nu, Gamma, NRIjac] = cons_dp1(constraint,obj, time,funtimes, qi, qdi, flags)
%% -------------------------------------------------------------------------
% INPUTS:
...
%     NRIjac = zeros(10,10);
    ...
    aj = Aj*ajBAR;
    ai = Ai*aiBAR;
    K_aib_aj = getK(aiBAR, aj);
    K_ajb_ai = getK(ajBAR,ai);
    NRIjac = lambda*[zeros(3,3) zeros(3,3) zeros(3,4) zeros(3,4);
        zeros(3,3) zeros(3,3) zeros(3,4) zeros(3,4); 
        zeros(4,3) zeros(4,3) K_aib_aj B_pi_aib'*B_pj_ajb;
        zeros(4,3) zeros(4,3) B_pj_ajb'*B_pi_aib K_ajb_ai];
end
%% Calculate B
    function [B] = getB(p,a)
        e = p(2:4);

function [ Phi, Phi_q, Nu, Gamma, NRIjac] = cons_d(constraint,obj, time,funtimes, qi, qdi, flags)
%% -------------------------------------------------------------------------
% INPUTS:
...
if flags(1)
    Phi = dij'*dij - funtimes.funt(id,time);
end
...
if flags(5)
    lambda = obj.myLambda(id,time);
%      K_sPi_c = getK(sPiBAR, c);
%     K_sQj_c = getK(sQjBAR, c);
%     NRIjac = lambda*[zeros(3,3) zeros(3,3) zeros(3,4) zeros(3,4);
%         zeros(3,3) zeros(3,3) zeros(3,4) zeros(3,4);
%         zeros(4,3) zeros(4,3) -K_sPi_c zeros(4,4);
%         zeros(4,3) zeros(4,3) zeros(4,4) K_sQj_c];
end
%% Calculate B
    function [B] = getB(p,a)
        e = p(2:4); 
    ...
    function [K] = getK(a,b)
        aTIL = [ 0 -a(3) a(2); a(3) 0 -a(1); -a(2) a(1) 0];
        bTIL = [ 0 -b(3) b(2); b(3) 0 -b(1); -b(2) b(1) 0];
        K = 2*[a'*b a'*bTIL; aTIL*b a*b'+b*a'-a'*b*eye(3)];
    end
end

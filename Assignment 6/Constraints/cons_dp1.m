function [ Phi, Phi_q, Nu, Gamma] = cons_dp1(constraint, time, qi, qdi, flags)
%% -------------------------------------------------------------------------
% INPUTS:
...
if flags(1)
    Phi = aiBAR'*(Ai')*Aj*ajBAR - funt(time);
end
...
if flags(4)
    ...
    B_pi_aib = getB(pi,aiBAR);
    B_pj_ajb = getB(pj,pjBAR);
    Gamma = -ajBAR'*Aj'*B_pid_aib*pid-aiBAR'*Ai'*Bpjd_ajb*pjd-...
            2*(pid'*B_pi_aib'*B_pj_ajb*pjd)+ funDDt;
end
%% Calculate B
    function [B] = getB(p,a)
        e = p(2:4); 

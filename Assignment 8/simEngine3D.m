...
%                 F(7*i-3:7*i) = 8*G'*Jbar*G*pd;
                F(3*i-2:3*i) = obj.myBodies(i).mass*grav;
                F(3*nb+(4*i-3):(3*nb+(4*i))) = 8*G'*Jbar*G*pd;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                         betah = h^2;
                        ...
                        rd = Cdr+(2/3)*h*rdd;
                        pd = Cdp+(2/3)*h*pdd;
%                         Cr = obj.myPosition(rindx,t-1)+obj.myVelocity(rindx,t-1)*h;
%                         Cp = obj.myPosition(pindx,t-1)+h*obj.myVelocity(pindx,t-1);
%                         Crd = obj.myVelocity(rindx,t-1);
%                         Cpd = obj.myVelocity(pindx,t-1);
%                         r = Cr+h.^2*rdd;
%                         p = Cp+h.^2*pdd;
%                         rd = Crd+h.^2*rdd;
%                         pd = Cpd+h.^2*pdd;
...
%                     obj.myVelocity(7*(1:nb)-3:7*(1:nb),t) = pd;
                    ...
                    for i = 1:nb
                        ...
                        grav = obj.myProblemID.gravity;
%                         F(7*i-6:7*i-4,1) = obj.myBodies(i).mass*grav;
%                         F(7*i-3:7*i) = 8*G'*Jbar*G*pd(4*i-3:4*i);
                        
                        F(3*i-2:3*i) = obj.myBodies(i).mass*grav;
                        F(3*nb+(4*i-3):(3*nb+(4*i))) = 8*G'*Jbar*G*pd(4*i-3:4*i);
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %Still need to implement Gen forces and
                        %torques
...
%                     norm(delz)
                    ...
                    if norm(delz)<=tol
                        if t == 2 %If 2nd timestep, use BDF1 approx 
                            r = obj.myPosition(rindx,1)+h*obj.myVelocity(rindx,1)+h^2*rdd;
                            p = obj.myPosition(pindx,1)+h*obj.myVelocity(pindx,1)+h^2*pdd;
                            rd = obj.myVelocity(rindx,1)+h*rdd;
                            pd = obj.myVelocity(pindx,1)+h*pdd;
                        else %If time>2, use BDF2
                            r = Cr+((2/3).^2*h.^2)*z(1:3*nb);
                            p = Cp+((2/3).^2*h.^2)*z(3*nb+1:7*nb);
                            rd = Cdr+(2/3)*h*z(1:3*nb);
                            pd = Cdp+(2/3)*h*z(3*nb+1:7*nb);
                        end
                        %Save all variables
